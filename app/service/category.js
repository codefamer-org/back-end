...
class MyService extends Service {
  async create() {
    ...
    try {
      const [data, isNew] = await ctx.model.Category.findOrCreate({
        where: {
  ...
  async destroy() {
    const { ctx, app } = this;
    try {
      const data = await ctx.model.Category.findByPk(ctx.params.id);
      if (data) {
  ...
  async update() {
    ...
    try {
      const data = await ctx.model.Category.update({ ...ctx.request.body, ...extParams }, { where: { id: ctx.params.id } });
      if (data?.[0] > 0) {
        ctx.helper.responseSuccessHelper({ data: '操作成功' });
      } else {
        ctx.helper.responseErrorHelper({ msg: '操作失败' });
  ...
  async getAll() {
    ...
    try {
      const data = await ctx.model.Category[sequelizeAPI]({
