...
class MyService extends Service {
  async create() {
    ...
    try {
      const [data, isNew] = await ctx.model.Article.findOrCreate({
        where: {
  ...
  async update() {
    ...
    try {
      const data = await ctx.model.Article.update({ ...ctx.request.body, ...extParams }, { where: { id: ctx.params.id } });
      if (data?.[0] > 0) {
        ctx.helper.responseSuccessHelper({ data: '操作成功' });
      } else {
        ctx.helper.responseErrorHelper({ msg: '操作失败' });
  ...
  async getAll() {
    ...
    try {
      const data = await ctx.model.Article[sequelizeAPI]({
        where: { ...ctx.helper.getWhereSql({ fileds: ['title', 'html', 'markdown']}) },
